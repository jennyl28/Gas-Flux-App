...
# --------------------------------------------------
# ✅ MAIN DASHBOARD (AVERAGED)
# --------------------------------------------------

def run():

    st.title("Element ratios averaged vs. time")
    ...
    if uploaded_file:
        ...
        # ✅ FORCE NUMERIC CLEANLY
        df_numeric = df.copy()

        for col in df_numeric.columns:
            if col not in ["StartDate", "Year", "Site"]:
        ...
        for i in range(3):
            ...
            with col1:
                num = st.selectbox(f"Numerator {i+1}", ["None"] + numeric_cols, key=f"n_avg{i}")

            with col2:
                den = st.selectbox(f"Denominator {i+1}", ["None"] + numeric_cols, key=f"d_avg{i}")
        ...
        ratio_df = ratio_df.groupby(
            ["Site", "Year"],
            as_index=False
        )[ratio_cols].mean()

        ratio_df = ratio_df.sort_values(by=["Site", "Year"])
        ...
        for num, den in ratio_pairs:
            ...
            for site in ratio_df["Site"].unique():
                sub_df = ratio_df[ratio_df["Site"] == site]

                fig.add_trace(go.Scatter(
                    x=sub_df["Year"],
                    y=sub_df[ratio_name],
                    mode="lines+markers",
                    name=f"{ratio_name}, {site}"
                ))

        title = " vs ".join([f"{n}/{d}" for n, d in ratio_pairs])

        fig.update_layout(
            title=dict(text=title,font=dict(size=24, color="black")),
            xaxis=dict(title=dict(text="Year", font=dict(size=18, color="black")), tickfont=dict(size=18, color="black")),
            yaxis=dict(title=dict(text="Element ratio", font=dict(size=18, color="black")), tickfont=dict(size=18, color="black")),
