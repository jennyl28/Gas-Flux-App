...
# --------------------------------------------------
# CLEANING FUNCTIONS (leave outside run)
# --------------------------------------------------
...
def convert_numeric(df):
    return df.applymap(clean_detection_limits).apply(pd.to_numeric, errors="ignore")
...
def run():
    ...
    st.title("Element ratios vs time")
    ...
    if uploaded_file:
        ...
        ratio_df = ratio_df.sort_values(by=["Site", "StartDate"])
        ...
        fig.update_layout(
            ...
            xaxis=dict(title=dict(text="Date", font=dict(size=18, color="black")), tickfont=dict(size=18, color="black")),
